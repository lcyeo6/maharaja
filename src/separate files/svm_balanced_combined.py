...
def run_svm(filtered_dataset):
    ...
    while false_ans == True:
        ...
        if user_input == 1 or user_input == 2:
            false_ans = False
            break
    tfidf_vectorizer = TfidfVectorizer(tokenizer = identity_token, ngram_range = (1, user_input), lowercase = False)
    ...
    for train_index, test_index in kfold.split(vectorized_data, equalized_sentiment):
        n += 1
        print(n)
        t3 = datetime.datetime.now()
        X_train, X_test = vectorized_data[train_index], vectorized_data[test_index]
        y_train = [equalized_sentiment[i] for i in train_index]
